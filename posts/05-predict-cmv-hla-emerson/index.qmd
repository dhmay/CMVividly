---
title: Predict CMV status and HLA type
date: "2026-08-10"
...
* Among CMV+ donors, the CMV ECOcluster can be used to predict HLA allele status for many alleles with high sensitivity and specificity
...
```{python}
metadata_filepath = "/Users/damonmay/CMVividly/data/processed/emerson_cohorts_sample_metadata.tsv"
pdf_sample_metadata = pd.read_csv(metadata_filepath, sep="\t")
...
```{python}
file_path_inters_2026 = "/Users/damonmay/CMVividly/data/processed/emerson_cmveco_2026_intersection_cdr3_exact.parquet"
pdf_tcr_matches_2026 = pd.read_parquet(file_path_inters_2026)
...
for allele in alleles_enough_cmvpos_with_hlacocluster:    
    ...
    pdf_rowcounts_this_allele_withmeta = pdf_sample_metadata_withhla_cmvpos.merge(
...
ax.set_title("ROC curves predicting HLA allele status\nusing breadth, by allele")
...
```{python}
f, ax = plt.subplots()
sns.scatterplot(data=pdf_allele_aurocs, y="auroc_depth", x="auroc_breadth")
